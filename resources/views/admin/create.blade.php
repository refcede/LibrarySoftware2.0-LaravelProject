...
<body class="bg-black text-white p-6 md:p-10 flex justify-center min-h-screen items-center">
    ...
    <div class="w-full max-w-3xl bg-zinc-900 p-8 rounded-xl border border-zinc-800 shadow-2xl">
        ...
        <div class="flex justify-between items-center mb-8 border-b border-zinc-800 pb-4">
            <div>
                <h2 class="text-2xl font-bold text-white">Tambah Software</h2>
                <p class="text-zinc-500 text-sm">Masukkan data software baru ke database.</p>
            </div>
        ...
        <form action="{{ route('admin.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" placeholder="Contoh: Adobe Photoshop 2024" 
                       class="w-full p-3 bg-black border border-zinc-700 rounded-lg text-white focus:outline-none focus:border-white transition-colors" required>
            </div>

            <div class="bg-zinc-950 p-4 rounded border border-zinc-800">
                <label class="block mb-2 text-sm font-medium text-zinc-400">Upload Icon / Logo</label>
                <input type="file" name="image" class="w-full text-sm text-zinc-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-bold file:bg-zinc-800 file:text-white hover:file:bg-zinc-700 cursor-pointer" accept=".svg,.png,.jpg,.webp" required>
                <p class="text-xs text-zinc-600 mt-2">*Wajib. Format: SVG, PNG, WebP (Max 2MB)</p>
            </div>
            
            <div>
                <label class="block mb-2 text-sm font-medium text-zinc-400">Deskripsi Singkat</label>
                <textarea name="description" rows="3" class="w-full p-3 bg-black border border-zinc-700 rounded-lg text-white focus:outline-none focus:border-white" required>{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="version" value="{{ old('version') }}" placeholder="Contoh: v1.0.5" class="w-full p-3 bg-black border border-zinc-700 rounded-lg text-white focus:outline-none focus:border-white" required>
                </div>
            </div>

            <div class="bg-zinc-950 p-4 rounded border border-zinc-800">
                <label class="block mb-2 text-sm font-medium text-zinc-400">Upload Screenshot Aplikasi</label>
                ...
                <p class="text-xs text-zinc-600 mt-2">*Opsional. Max 4MB.</p>
            </div>

            <div>
                <label class="block mb-1 text-zinc-400 text-sm">Panduan Instalasi</label>
                <textarea name="install_instructions" rows="4" class="w-full p-3 bg-black border border-zinc-700 rounded text-white focus:outline-none focus:border-white">{{ old('install_instructions') }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block mb-1 text-zinc-400 text-sm">Ukuran File</label>
                    <input type="text" name="file_size" value="{{ old('file_size') }}" class="w-full p-3 bg-black border border-zinc-700 rounded text-white focus:outline-none focus:border-white">
                </div>
                <div>
                    <label class="block mb-1 text-zinc-400 text-sm">Support OS</label>
                    <input type="text" name="os_support" value="{{ old('os_support') }}" class="w-full p-3 bg-black border border-zinc-700 rounded text-white focus:outline-none focus:border-white">
                </div>
            </div>

            <div>
                <label class="block mb-2 text-sm font-medium text-zinc-400">Link Download</label>
                <input type="url" name="download_url" value="{{ old('download_url') }}" placeholder="https://..." class="w-full p-3 bg-black border border-zinc-700 rounded-lg text-white focus:outline-none focus:border-white" required>
            </div>

            <div class="flex items-center gap-3 p-4 border border-zinc-700 rounded bg-zinc-900/50 hover:bg-zinc-800 transition-colors">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" class="w-5 h-5 accent-blue-600 cursor-pointer rounded" {{ old('is_featured') ? 'checked' : '' }}>
